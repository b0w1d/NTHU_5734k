struct WeightGraph {
  ...
  void set_slack(int x) {
    slack[x] = 0;
    for (int u = 1; u <= n; ++u)
      if (g[u][x].w > 0 and st[u] != x and S[st[u]] == 0) update_slack(u, x);
  }
  void q_push(int x) {
    if (x <= n)
      q.push(x);
  ...
  void add_blossom(int u, int lca, int v) {
    ...
    for (size_t i = 0; i < flo[b].size(); ++i) {
      int xs = flo[b][i];
      for (int x = 1; x <= n_x; ++x)
        if (g[b][x].w == 0 or e_delta(g[xs][x]) < e_delta(g[b][x]))
  ...
  bool matching() {
    ...
    q = queue<int>();
    for (int x = 1; x <= n_x; ++x)
      if (st[x] == x and not match[x]) pa[x] = 0, S[x] = 0, q_push(x);
    ...
    for (;;) {
      ...
      for (int b = n + 1; b <= n_x; ++b)
        if (st[b] == b and S[b] == 1) d = min(d, lab[b] / 2);
      for (int x = 1; x <= n_x; ++x)
      ...
      q = queue<int>();
      for (int x = 1; x <= n_x; ++x)
        if (st[x] == x and slack[x] and st[slack[x]] != x and
            e_delta(g[slack[x]][x]) == 0)
          if (on_found_edge(g[slack[x]][x])) return true;
      for (int b = n + 1; b <= n_x; ++b)
        if (st[b] == b and S[b] == 1 and lab[b] == 0) expand_blossom(b);
    }
    return false;
  }
  pair<long long, int> solve() {
    ...
    for (int u = 1; u <= n; ++u)
      if (match[u] and match[u] < u) tot_weight += g[u][match[u]].w;
    return {tot_weight, n_matches};
  }
  void add_edge(int ui, int vi, int wi) { g[ui][vi].w = g[vi][ui].w = wi; }
  void init(int _n) {  // 1-index, zero indicates unsaturated
    n = _n;
    for (int u = 1; u <= n; ++u)
