...
struct BigNum {
  ...
  BigNum(const vector<ll> &v, ll b = 10000) : sign(1), B(b), BW(ceil(log10(b))), cells(v) {}
  ...
  friend bool operator==(const BigNum &a, const BigNum &b) { return a.sign == b.sign && a.cells == b.cells; }
  ...
  static vector<ll> add(const vector<ll> &a, const vector<ll> &b, int al = -1, int ar = -1, int bl = -1, int br = -1) {
    if (al == -1) al = 0, ar = a.size(), bl = 0, br = b.size();
    vector<ll> c(max(ar - al, br - bl));
    for (int i = 0; i < c.size(); ++i)
      c[i] = (al + i < a.size() ? a[al + i] : 0) + (bl + i < b.size() ? b[bl + i] : 0);
    return c;
  }
  static vector<ll> sub(const vector<ll> &a, const vector<ll> &b, int al = -1, int ar = -1, int bl = -1, int br = -1) {
    if (al == -1) al = 0, ar = a.size(), bl = 0, br = b.size();
    vector<ll> c(max(ar - al, br - bl));
    for (int i = 0; i < c.size(); ++i)
      c[i] = (al + i < a.size() ? a[al + i] : 0) - (bl + i < b.size() ? b[bl + i] : 0);
  ...
  friend BigNum operator*(BigNum x, BigNum y) {
    ...
    int nn = 31 - __builtin_clz(int(x.cells.size())) + (__builtin_popcount(int(x.cells.size())) > 1);
    function<vector<ll>(const vector<ll> &, const vector<ll> &, int, int, int, int)>
        karatsuba = [&](const vector<ll> &a, const vector<ll> &b, int al, int ar, int bl, int br) {
          if (al + 256 >= ar) {
            vector<ll> r(ar - al << 1);
  ...
  friend ostream& operator<<(ostream &os, BigNum x) {
    ...
    for (auto it = x.cells.rbegin(); it != x.cells.rend(); ++it) {
      if (it == x.cells.rbegin()) os << *it;
      else os << setw(x.BW) << setfill('0') << *it;
    }
    return os;
  }
...
signed main() {
  ...
  BigNum ab("1");
  for (BigNum i; i < b; i = i + BigNum("1")) ab = ab * a;

  BigNum ba("1");
  for (BigNum i; i < a; i = i + BigNum("1")) ba = ba * b;
